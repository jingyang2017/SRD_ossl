...
class TIN_loader(torch.utils.data.Dataset):
    def __init__(self, data_folder, transform):
        ...
        self.img_list = [f for f in glob(
            data_path + "**/*.JPEG", recursive=True)]
        self.img_list.sort()
...
def get_cifar100_dataloaders(batch_size=128, num_workers=8,
    ...
    if ood == 'tin':
        ood_set = TIN_loader(
            data_folder, transform=TransformTwice(tiny_transform))
    elif ood == 'places':
        ood_set = Places365Instance(
            data_folder + '/places365', transform=TransformTwice(tiny_transform))
