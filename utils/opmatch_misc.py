...
def exclude_dataset(args, dataset, model, exclude_known=False):
    ...
    # dataset.init_index()
    test_loader = DataLoader(dataset, batch_size=args.batch_size,num_workers=args.num_workers,drop_last=False, shuffle=False)
    ...
    with torch.no_grad():
        for batch_idx, ((_, _, inputs), targets) in enumerate(test_loader):
            ...
            outputs, outputs_open = model(inputs, is_feat=True)
            outputs = F.softmax(outputs, 1)
            #YJ: outputs_open decides Bx2XC (2:0 or 1)
            #dim=1: {0,1}={in,out}
            ...
            # label 0 confidence <0.5 then this sample is in ind
